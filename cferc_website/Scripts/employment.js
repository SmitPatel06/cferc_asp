...
app.controller("employment", function ($scope, $http) {
    ...
    //send initial data
    $http.post('Employment/data', { areaName: $scope.areaName, year: $scope.selectedYear })
    ...
    var margin = { top: 4, right: 4, bottom: 6, left: 50 },
    ...
    function clicked(d) {
        ...
        if (d && centered !== d) {
            ...
            $http.post('Employment/data', dataToSend)
                .success(function (response) {
                    $scope.data = response;
                    updatePie(response);
